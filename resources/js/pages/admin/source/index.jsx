...
export default function Index({ sources, meta, filters: initialFilters }) {
    ...
    useEffect(() => {
        router.get(route('sources.index'), filters, {
            preserveState: true,
            replace: true,
            preserveScroll: true,
        });
    ...
    const columns = [{ key: 'name', label: 'Lead Source' }];
    ...
    const submitSource = (e) => {
        ...
        if (name.length < 3) {
            toast.error('Lead Source name must be at least 3 characters');
            return;
        }

        router.post(route('sources.store'), { name }, {
            onSuccess: () => {
                setShowModal(false);
                setName('');
            },
                ...
                {showModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div className="w-[400px] rounded-lg bg-white p-6 shadow-lg">
                            <h2 className="mb-4 text-lg font-semibold">Create Lead Source</h2>

                            <form onSubmit={submitSource}>
                                <div className="mb-4">
                                    <label className="mb-1 block text-sm font-medium">Lead Source Name</label>
