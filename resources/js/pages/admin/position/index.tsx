...
export default function Index({ positions, meta, filters: initialFilters }) {
    ...
    const columns = [{ key: 'name', label: 'Contact Role' }];

    const submitPosition = (e) => {
        e.preventDefault();

        if (!name || name.trim() === '') {
            toast.error('Contact role name is required');
            return;
        }

        if (name.length < 3) {
            toast.error('Contact role name must be at least 3 characters');
            return;
        }

        router.post(route('positions.store'), { name }, {
            onSuccess: () => {
                setShowModal(false);
                setName('');
            },
                ...
                {showModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div className="w-[400px] rounded-lg bg-white p-6 shadow-lg">
                            <h2 className="mb-4 text-lg font-semibold">Create Contact Role</h2>

                            <form onSubmit={submitPosition}>
                                <div className="mb-4">
                                    <label className="mb-1 block text-sm font-medium">Contact Role Name</label>
